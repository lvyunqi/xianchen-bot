...
func dispatchEvent(msg IPCMessage) (shutdown bool, result IPCMessage) {
	...
	if msg.Event == "group_message" || msg.Event == "private_message" {
		writeRuntimeLog("IPC收到消息事件", fmt.Sprintf("event=%s id=%s args=%d", msg.Event, msg.ID, len(msg.ArgsB64)))
	}
	if msg.Event == "stop" {
		closeSettingsWindow()
		return true, result
...
func runWorker(input io.Reader, output io.Writer) error {
	...
	client := newIPCClientForTest(writeMessage, readMessage)
	deferredEvents := make([]IPCMessage, 0, 4)
	client.handleInterleaved = func(message IPCMessage) error {
		deferredEvents = append(deferredEvents, message)
		return nil
	}
	setCurrentAPITransport(client)
	...
	for {
		...
		for len(deferredEvents) > 0 {
			deferred := deferredEvents[0]
			deferredEvents = deferredEvents[1:]
			deferredShutdown, deferredResult := dispatchEvent(deferred)
			shutdown = shutdown || deferredShutdown
			if err := client.writeMessage(deferredResult); err != nil {
				return err
			}
		}
		if shutdown {
