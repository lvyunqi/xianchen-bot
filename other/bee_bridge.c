...
#ifndef BEE_WORKER_SHA256_VALID
#error "plugin_config.h is missing the Worker SHA-256 validity marker"
#elif BEE_WORKER_SHA256_VALID != 1
#error "refusing to build a publishable DLL without a valid Worker SHA-256"
#endif

#ifndef BEE_WORKER_SHA256_SIZE
#error "plugin_config.h is missing the Worker SHA-256 size"
#elif BEE_WORKER_SHA256_SIZE != 32
#error "Worker SHA-256 must contain exactly 32 bytes"
#endif
...
static void trace_ipc(const char *label, const char *line) {
    char enabled[4];
    FILE *file;
    if (!GetEnvironmentVariableA("XIANLV_IPC_TRACE", enabled, sizeof(enabled))) return;
    file = fopen("xianlv_ipc_trace.log", "ab");
    if (!file) return;
    fprintf(file, "%s %s\r\n", label ? label : "ipc", line ? line : "");
    fclose(file);
}
...
static BOOL secure_bytes_equal(const unsigned char *left, const unsigned char *right, DWORD size) {
    DWORD index;
    unsigned char difference = 0;
    for (index = 0; index < size; index++) difference |= (unsigned char)(left[index] ^ right[index]);
    return difference == 0;
}

static BOOL verify_worker_resource(const void *bytes, DWORD size) {
    ...
    DWORD digest_size = 0;
    DWORD result_size = 0;
    unsigned char digest[BEE_WORKER_SHA256_SIZE];
    BOOL valid = FALSE;
    if (!bytes || !size) return FALSE;
    if (BCryptOpenAlgorithmProvider(&algorithm, BCRYPT_SHA256_ALGORITHM, NULL, 0) < 0) goto done;
    if (BCryptGetProperty(algorithm, BCRYPT_OBJECT_LENGTH, (PUCHAR)&hash_object_size, sizeof(hash_object_size), &result_size, 0) < 0 ||
        result_size != sizeof(hash_object_size) || hash_object_size == 0) goto done;
    if (BCryptGetProperty(algorithm, BCRYPT_HASH_LENGTH, (PUCHAR)&digest_size, sizeof(digest_size), &result_size, 0) < 0 ||
        result_size != sizeof(digest_size) || digest_size != BEE_WORKER_SHA256_SIZE) goto done;
    hash_object = (PUCHAR)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, hash_object_size);
    if (!hash_object) goto done;
    if (BCryptCreateHash(algorithm, &hash, hash_object, hash_object_size, NULL, 0, 0) < 0) goto done;
    if (BCryptHashData(hash, (PUCHAR)bytes, size, 0) < 0) goto done;
    if (BCryptFinishHash(hash, digest, sizeof(digest), 0) < 0) goto done;
    valid = secure_bytes_equal(digest, BEE_WORKER_SHA256, BEE_WORKER_SHA256_SIZE);
done:
...
static BOOL extract_worker(void) {
    ...
    if (!bytes || !size) return FALSE;
    if (!verify_worker_resource(bytes, size)) return FALSE;
    ...
    if (!WriteFile(file, bytes, size, &written, NULL) || written != size) {
        CloseHandle(file);
        DeleteFileW(g_worker_path);
        return FALSE;
    }
    FlushFileBuffers(file);
    CloseHandle(file);
    SetFileAttributesW(g_worker_path, FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_TEMPORARY);
    return TRUE;
}
...
static BOOL read_line_timeout(char **line_out, DWORD timeout_ms) {
    size_t capacity = 4096, length = 0;
    ULONGLONG deadline = GetTickCount64() + timeout_ms;
    char *line = (char *)HeapAlloc(GetProcessHeap(), 0, capacity);
    if (!line) return FALSE;
    for (;;) {
        char ch; DWORD got = 0, available = 0;
        if (!PeekNamedPipe(g_stdout_read, NULL, 0, NULL, &available, NULL)) {
            HeapFree(GetProcessHeap(), 0, line);
            return FALSE;
        }
        if (!available) {
            if (!worker_running() || GetTickCount64() >= deadline) {
                HeapFree(GetProcessHeap(), 0, line);
                return FALSE;
            }
            Sleep(2);
...
static void abandon_worker(void) {
    close_handle(&g_stdin_write);
    if (g_process && WaitForSingleObject(g_process, 0) == WAIT_TIMEOUT) TerminateProcess(g_process, 0);
    if (g_process) WaitForSingleObject(g_process, 1000);
    close_handle(&g_stdout_read);
    close_handle(&g_process);
    if (g_worker_path[0]) {
        SetFileAttributesW(g_worker_path, FILE_ATTRIBUTE_NORMAL);
        DeleteFileW(g_worker_path);
    }
}
...
static BOOL service_api_call(const char *line, const char *robot_json) {
    ...
    if (!json_string_field(line, "api", &api_text) &&
        !json_string_field(robot_json ? robot_json : "", "api", &api_text)) {
        send_api_result(id, NULL, "api_call missing robot api address"); goto done;
    }
...
static int send_event(const char *event, int argc, const char **argv) {
    ...
    for (;;) {
        ...
        if (!read_line_timeout(&line, IPC_TIMEOUT_MS)) goto done;
        trace_ipc("read", line);
        ...
        if (strstr(line, "\"type\":\"event_result\"") &&
            json_string_field(line, "id", &response_id) &&
            strcmp(response_id, expected_id) == 0) {
            HeapFree(GetProcessHeap(), 0, response_id);
            completed = TRUE;
            break;
        }
        if (response_id) HeapFree(GetProcessHeap(), 0, response_id);
        HeapFree(GetProcessHeap(), 0, line);
        line = NULL;
    }
    if (completed) result = parse_result(line);
done:
    if (line) HeapFree(GetProcessHeap(), 0, line);
    if (!completed) abandon_worker();
    LeaveCriticalSection(&g_lock);
    return result;
}

static BOOL queue_event_async(const char *event, int argc, const char **argv) {
    char header[256], tail[] = "]}\n";
    DWORD id;
    int i;
    BOOL queued = FALSE;
...
BEE_EXPORT const char *BEE_CALL Bee_Init_Internal(const char *robot) {
	/* Queue initialization without waiting for database work. Bee versions that
	   defer the Enable callback still get a ready settings page, while metadata
	   discovery remains responsive. A later Enable event is idempotent. */
	const char *a[] = {robot};
	queue_event_async("enable", 1, a);
...
BEE_EXPORT void BEE_CALL Bee_Unload_Internal(const char *robot) { const char *a[] = {robot}; if (worker_running()) send_event("unload", 1, a); stop_worker("stop"); }
