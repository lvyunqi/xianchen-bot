...
func ImageDownloadURL(message string) string {
	...
	// Bee versions have emitted CQ-style tags, bracketed image tags and JSON
	// fragments over time.  Accept all of them, but only return an HTTP(S)
	// address so ordinary text cannot accidentally be uploaded.
	markers := []string{",url=", "url=", `"url":"`, `"url":`, "image_url="}
	...
	for _, marker := range markers {
		...
		candidate := imageURLToken(text[index+len(marker):])
		if candidate != "" {
			return candidate
		}
	}

	// Some adapters pass the image URL as a standalone message value.
	for _, prefix := range []string{"https://", "http://"} {
		if index := strings.Index(lower, prefix); index >= 0 {
			if candidate := imageURLToken(text[index:]); candidate != "" {
				return candidate
			}
		}
	}
...
func imageURLToken(value string) string {
...
func (ctx *RobotContext) Call(op int, args ...string) (string, error) {
	...
	var result []byte
	var err error
	if transport, ok := ctx.transport.(interface {
		CallWithAPI(string, []byte) ([]byte, error)
	}); ok {
		result, err = transport.CallWithAPI(ctx.APIText, []byte(command))
	} else {
...
type IPCClient struct {
	...
	read              messageReader
	nextID            uint64
	handleInterleaved func(IPCMessage) error
}
...
func (client *IPCClient) writeMessage(message IPCMessage) error {
	client.writeMu.Lock()
...
func (client *IPCClient) Call(command []byte) ([]byte, error) {
	return client.CallWithAPI("", command)
}

func (client *IPCClient) CallWithAPI(apiText string, command []byte) ([]byte, error) {
	...
	request := IPCMessage{Type: "api_call", ID: id, APIText: strings.TrimSpace(apiText), CommandB64: base64.StdEncoding.EncodeToString(command)}
	...
	for {
		...
		if response.Type == "event" && client.handleInterleaved != nil {
			if err := client.handleInterleaved(response); err != nil {
				return nil, fmt.Errorf("处理插入事件 %q: %w", response.ID, err)
			}
			continue
		}
